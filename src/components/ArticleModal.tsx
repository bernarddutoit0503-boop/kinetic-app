...
export const ArticleModal = ({ article, onClose, isBookmarked, onBookmarkToggle }: ArticleModalProps) => {
  const [summary, setSummary] = useState<string | null>(null);
  const [loadingSummary, setLoadingSummary] = useState(false);

  const handleGenerateSummary = async () => {
    setLoadingSummary(true);
    // Pass the available description as content — summarizeArticle is optimised
    // to work from short context and expands using model knowledge.
    ...
    <>
      ...
      >
        ...
        <div className="flex justify-center pt-3 pb-1 shrink-0">
          <div className="w-10 h-1 bg-outline-variant rounded-full" aria-hidden="true"></div>
        </div>
